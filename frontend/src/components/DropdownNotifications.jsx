...
import { getPegawaiNotifications } from "../pages/pegawai/services/pegawaiService";
...
function DropdownNotifications({ align }) {
  ...
  useEffect(() => {
    if (user?.role !== "pegawai_sarpras") return;
  ...
  const hasUnread = user?.role === "pegawai_sarpras" && notifications.length > 0;
          ...
          <ul className="max-h-96 overflow-y-auto">
            {user?.role === "pegawai_sarpras" ? (
              loading ? (
                ...
                <li className="px-4 py-3 text-sm text-gray-500">Tidak ada notifikasi</li>
              ) : (
                notifications.map((n) => (
                  <li
                    key={n.id}
