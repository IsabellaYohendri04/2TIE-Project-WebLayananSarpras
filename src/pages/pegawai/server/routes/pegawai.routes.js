...
router.use(authenticate, authorize("pegawai_sarpras"));

const sanitizePegawai = (user) => ({
  id: user.id,
  nip: user.nip,
  nama: user.nama,
  email: user.email,
  ...
  try {
    ...
    if (!nip || !nama || !email || !password || !jabatan) {
      return res.status(400).json({
        success: false,
        message: "NIP, nama, email, password, dan jabatan wajib diisi",
      });
    }
    ...
    const [result] = await pool.query(
      `INSERT INTO users (email, password, role, nama, nip, jabatan, divisi, no_hp, status)
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (!nip || !nama || !email || !jabatan) {
      return res.status(400).json({
        success: false,
        message: "NIP, nama, email, dan jabatan wajib diisi",
      });
    }
    ...
    if (password) {
      const hashedPassword = await bcrypt.hash(password, 10);
      await pool.query(
        `UPDATE users SET nip = ?, nama = ?, email = ?, password = ?, jabatan = ?, divisi = ?, no_hp = ?, status = ?
      ...
      await pool.query(
        `UPDATE users SET nip = ?, nama = ?, email = ?, jabatan = ?, divisi = ?, no_hp = ?, status = ?
         WHERE id = ? AND role = 'pegawai_sarpras'`,
        [
          nip,
          nama,
