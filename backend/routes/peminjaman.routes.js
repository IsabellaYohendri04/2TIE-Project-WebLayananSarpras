...
function parseCreateBody(req) {
  ...
  const fileProposal = req.file
    ? `/uploads/peminjaman/${req.file.filename}`
    : null;
...
async function resolvePeminjamEmail(pool, row) {
  ...
  if (row.nim) {
    const [users] = await pool.query("SELECT email FROM users WHERE nim = ?", [
      row.nim,
    ]);
    if (users[0]?.email) return users[0].email;
  }
...
const tipeConfig = {
  barang: {
    ...
    itemCol: "barang",
    format: formatBarangRow,
  },
  ruangan: {
    table: "peminjaman_ruangan",
    itemCol: "ruangan",
    format: formatRuanganRow,
  },
  laboratorium: {
    table: "peminjaman_laboratorium",
    itemCol: "laboratorium",
    format: formatLabRow,
  },
};
...
function attachPeminjamanRoutes(app) {
    ...
    async (req, res) => {
      try {
        ...
        for (const q of queries) {
          const [rows] = param
            ? await pool.query(q, [param])
            : await pool.query(q.replace(filter, ""));
          results.push(...rows);
        }
      ...
      try {
        ...
        if (!nama || !barang || !tanggalPinjam) {
          return res
            ...
            .json({
              success: false,
              message: "Nama, barang, dan tanggal wajib diisi",
            });
        }
        const pool = getPool();
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        const pool = getPool();
        await pool.query("DELETE FROM peminjaman_barang WHERE id = ?", [
          req.params.id,
        ]);
  ...
  app.get(
    ...
    async (_req, res) => {
      try {
        const pool = getPool();
        const [rows] = await pool.query(
          "SELECT * FROM peminjaman_ruangan ORDER BY created_at DESC",
        );
      ...
      try {
        ...
        if (!nama || !ruangan || !tanggalPinjam) {
          return res
            ...
            .json({
              success: false,
              message: "Nama, ruangan, dan tanggal wajib diisi",
            });
        }
        const pool = getPool();
        const [result] = await pool.query(
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        const pool = getPool();
        await pool.query("DELETE FROM peminjaman_ruangan WHERE id = ?", [
          req.params.id,
        ]);
        res.json({ success: true });
      } catch (error) {
  ...
  app.get(
    ...
    async (_req, res) => {
      try {
        const pool = getPool();
        const [rows] = await pool.query(
          "SELECT * FROM peminjaman_laboratorium ORDER BY created_at DESC",
        );
      ...
      try {
        ...
        if (!nama || !laboratorium || !tanggalPinjam) {
          return res
            ...
            .json({
              success: false,
              message: "Nama, laboratorium, dan tanggal wajib diisi",
            });
        }
        const pool = getPool();
        const [result] = await pool.query(
        ...
        const [rows] = await pool.query(
          "SELECT * FROM peminjaman_laboratorium WHERE id = ?",
          [result.insertId],
        );
  ...
  app.put(
    "/api/peminjaman/laboratorium/:id",
    authenticate,
    adminOnly,
    async (req, res) => {
      try {
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        const pool = getPool();
        await pool.query("DELETE FROM peminjaman_laboratorium WHERE id = ?", [
          req.params.id,
        ]);
        res.json({ success: true });
      } catch (error) {
  ...
  app.get(
    "/api/peminjaman/laporan-kondisi",
    authenticate,
    submitRoles,
    async (req, res) => {
      try {
        ...
        for (const [tipe, { table, itemCol }] of Object.entries(
          laporanTables,
        )) {
          let query = `
          SELECT * FROM ${table}
        ...
        results.sort(
          (a, b) => new Date(b.tanggalKembali) - new Date(a.tanggalKembali),
        );
        res.json(results);
      } catch (error) {
        res.status(500).json({ success: false, message: error.message });
      ...
      try {
        ...
        if (!peminjamanId || !tipe) {
          return res
            ...
            .json({
              success: false,
              message: "ID peminjaman dan tipe wajib diisi",
            });
        }
        ...
        if (!config) {
          return res
            .status(400)
            .json({ success: false, message: "Tipe peminjaman tidak valid" });
        }
        ...
        res.json({
          success: true,
          message: "Laporan kondisi berhasil dikirim",
        });
      } catch (error) {
        res.status(500).json({ success: false, message: error.message });
      }
